--TEST--
PHPT_Assert_NotRegExp verifies that the pattern PCRE pattern matches the value that is passed
...
$valid = new PHPT_Assert_NotRegExp('/\d/', 'ABCabc');
assert('$valid->getStatus()');
echo $valid->getMessage() . "\n";

$not_valid = new PHPT_Assert_NotRegExp('/\d/', 'ABCabc123');
