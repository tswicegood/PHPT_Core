--TEST--
PHPT_Assert_NotEqual tests two values to see if they are not equal (!=).  getStatus()
...
$equal = new PHPT_Assert_NotEqual(123, 321);
assert('$equal->getStatus()');
echo $equal->getMessage() . "\n";

$not_equal = new PHPT_Assert_NotEqual(123, 123);
