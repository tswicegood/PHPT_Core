--TEST--
An optional second parameter of PHPT_Assert_Null::__construct() allows for a custom
...
$valid = new PHPT_Assert_Null(null, 'value should be null');
assert('$valid->getStatus()');
echo $valid->getMessage() . "\n";

$not_valid  = new PHPT_Assert_Null(123, 'value should be null');
assert('!$not_valid->getStatus()');
echo $not_valid->getMessage() . "\n";

$test = new PHPT_Assert_Null(null, 'value one: %s');
echo $test->getMessage() . "\n";
unset($test);

$test = new PHPT_Assert_Null(null, 'should be is: [%2$s]');
echo $test->getMessage() . "\n";
unset($test);

$test = new PHPT_Assert_Null(123, 'should be is not: [%2$s]');
echo $test->getMessage() . "\n";
unset($test);
